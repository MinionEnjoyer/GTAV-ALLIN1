...
OPERATIONS = frozenset({
    ...
    "check_update", "read_garages", "check_sdk_update", "startup_status", "shutdown", "open_activity_folder", "open_launcher_release",
})
...
def serve(service, incoming, outgoing):
    ...
    while True:
        ...
        try:
            ...
            service.progress = lambda percentage, message: send(request_id, "progress", {"schema_version": 1, "event": "launcher.progress", "percentage": percentage, "message": message})
            operation = request["operation"]
            if not isinstance(operation, str) or operation not in OPERATIONS:
                raise ValueError("Invalid Launcher operation")
            if operation == "shutdown": send(request_id, "result", {"closed": True}); return
            with contextlib.redirect_stdout(sys.stderr):
                if operation == "review": result = service.review(payload)
            ...
            send(request_id, "error", {"message": str(error), "error_type": type(error).__name__})
