...
{
    /// <summary>
    /// Fail-closed authorization for fixed Harmony and Paleto interior bridges.
    /// The bridge is metadata only. It may be executed only by the garage
    /// entry coordinator while the screen is verified fully black.
    /// </summary>
    internal sealed class OfficialInteriorStockBridgePolicy
    {
        internal static readonly OfficialInteriorStockBridgePolicy Harmony =
            new OfficialInteriorStockBridgePolicy("harmony", "mpbattle", "dlc_mpBattle",
                "MPBATTLE_INTERIOR_ADDITIONS",
                new[] { "ba_int_placement_ba_interior_1_dlc_int_02_ba_milo_" }, false);
        internal static readonly OfficialInteriorStockBridgePolicy Paleto =
            new OfficialInteriorStockBridgePolicy("paleto", "mpvinewood", "dlc_mpVinewood",
                "mpVinewood_INTERIOR_ADDITIONS", new[] { "vw_casino_garage" }, true);

        internal static OfficialInteriorStockBridgePolicy For(DeferredMapProperty property) =>
            property == DeferredMapProperty.Harmony ? Harmony :
            property == DeferredMapProperty.Paleto ? Paleto : null;

        internal string Key { get; }
        internal string SourcePack { get; }
        internal string SourceDevice { get; }
        internal bool RequiresLoading { get; }
        internal string PackName => "allin1_" + SourcePack + "_" + Key + "_bridge";
        internal string DeviceName => "dlc_" + PackName;
        internal string MarkerName => PackName + ".active";
        internal string ReceiptName => PackName + ".runtime.json";
        internal string DormantGroup =>
            ("ALLIN1_STOCK_" + SourcePack + "_" + Key + "_V1").ToUpperInvariant();
        internal string StockChangeset { get; }
        internal string RuntimeContract =>
            "allin1-stock-" + SourcePack + "-" + Key + "-black-transition-v1";
        internal string[] Ipls { get; }

        private OfficialInteriorStockBridgePolicy(string key, string sourcePack,
            string sourceDevice, string changeset, string[] ipls, bool requiresLoading)
        {
            Key = key;
            SourcePack = sourcePack;
            SourceDevice = sourceDevice;
            StockChangeset = changeset;
            Ipls = ipls;
            RequiresLoading = requiresLoading;
        }

        private const int VerificationCacheMilliseconds = 5000;
        private readonly TimedBooleanVerificationCache
            ContractCache = new TimedBooleanVerificationCache();
        private readonly AsyncSourceArchiveAttestationCache
            NativeMutationAttestation =
                new AsyncSourceArchiveAttestationCache();

        internal bool IsCurrentRuntimeActivationAuthorized =>
            ContractCache.GetOrVerify(
                Environment.TickCount, VerificationCacheMilliseconds,
                VerifyInstalledContract);

        internal bool IsCurrentNativeMutationAuthorized
        {
            get
            {
        ...
        internal void WarmUpCurrentNativeMutationAuthorization()
        {
            if (!TryResolveInstalledContract(
                    out SourceArchiveAttestationIdentity identity))
                return;
            if (NativeMutationAttestation.Begin(identity, VerifySourceArchive))
            {
                ClientLog.Info("DeferredMap",
                    Key + "_native_attestation_warmup_started",
        ...
        private bool VerifyInstalledContract() =>
            TryResolveInstalledContract(
                out SourceArchiveAttestationIdentity _);

        private bool TryResolveInstalledContract(
            out SourceArchiveAttestationIdentity identity)
        {
            identity = null;
            try
            {
                ...
                return VerifyFiles(gameRoot, edition, out identity);
            }
            catch { return false; }
        }

        internal bool VerifyFiles(string gameRoot, string edition,
            out SourceArchiveAttestationIdentity identity)
        {
            identity = null;
            try
            {
                string packRoot = Path.Combine(gameRoot, "mods", "update", "x64", "dlcpacks", PackName);
                string archivePath = Path.Combine(packRoot, "dlc.rpf");
                string markerPath = Path.Combine(packRoot, MarkerName);
                string receiptPath = Path.Combine(packRoot, ReceiptName);
                ...
                if (!MarkerEquals(marker, "schema", "1") ||
                    ...
                    !MarkerEquals(marker, "property_scope", Key) ||
                    !MarkerEquals(marker, "activation",
                        "explicit-" + Key + "-entry-black-transition") ||
                ...
                if (receipt == null ||
                    ...
                    !ReceiptEquals(source, "pack", SourcePack) ||
                    ...
                    !ReceiptBooleanEquals(source,
                        "requires_loading_screen", RequiresLoading) ||
        ...
        private bool VerifySourceArchive(
            SourceArchiveAttestationIdentity identity)
        {
            ...
            ClientLog.Info("DeferredMap", verified
                    ? Key + "_native_attestation_warmup_verified"
                    : Key + "_native_attestation_warmup_rejected",
                new Dictionary<string, object>
                {
