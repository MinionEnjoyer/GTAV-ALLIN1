...
def _release_tree(tmp_path: Path) -> Path:
    ...
    (root / "README.md").write_text("Current public release: **0.4.5**\n")
    (root / "RELEASE_NOTES.md").write_text("# Release 0.4.5\n")

    files = {
        "src/allin1/__init__.py": b'__version__ = "0.4.5"\n',
    ...
    (root / "script/ALLIN1.csproj").write_text(
        "<Project><PropertyGroup><Version>0.4.5</Version>"
        "<AssemblyVersion>0.4.5.0</AssemblyVersion>"
        "<FileVersion>0.4.5.0</FileVersion></PropertyGroup></Project>"
    )
    return root
...
def test_release_verifier_rejects_structural_manifest_errors(tmp_path):
    ...
    with zipfile.ZipFile(unmatched, "w") as archive:
        archive.writestr("release.json", b'{"version":"0.4.5"}')
        archive.writestr("extra.txt", b"extra")
        archive.writestr("checksums.json", json.dumps({
            "release.json": hashlib.sha256(b'{"version":"0.4.5"}').hexdigest(),
        }))
    ...
    _write_manifest_archive(incomplete, {"release.json": b'{"version":"0.4.5"}'})
